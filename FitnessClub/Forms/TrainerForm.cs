...
namespace FitnessClub.Forms
{
    public partial class TrainerForm : Form
    {
        ...
        public void UpdateWeekSchedule()
        {
            ...
            {
                data = FitnessClubDbContext.DbContext.Workouts.Where(w => w.DateWorkout >= monday && w.DateWorkout.Date <= sunday && w.Trainer == User.SignedUser.Trainer);
            }
        ...
        private void TypeScheduleComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (TypeScheduleComboBox.SelectedIndex == 0)
            {
                TrainerFilterComboBox.Enabled = true;
            }
            else
            {
                TrainerFilterComboBox.SelectedItem = null;
                TrainerFilterComboBox.Enabled = false;
            }
            UpdateWeekSchedule();
        }
